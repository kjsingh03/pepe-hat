...
const App = () => {
  ...
  useEffect(() => {
    if (hat.width > 0 && hat.height > 0) {
      setHat(prevHat => ({ ...prevHat })); // Force re-render
    }
      ...
      <div className={`h-screen z-40 bg-[url(./assets/mbg1.png)] md:bg-[url(./assets/bg1.webp)] bg-no-repeat bg-center bg-cover bg-fixed flex items-center justify-center`}>
        <div className="flex flex-col border-slate-400/20 border md:border-none h-[15rem] text-white rounded-lg w-[90%] md:w-[35rem] justify-center items-center gap-6">
          <p className="text-center text-xl md:text-2xl font-semibold w-[95%]">
            Upload an image to add pepe hat
          </p>
          <label htmlFor="file" className="text-white cursor-pointer px-5 rounded-3xl py-3 bg-[#32A430] font-semibold">
            Upload an image
          </label>
          <input
            ref={fileInputRef}
          ...
          {image && (
            <div className="screen z-50 w-screen h-screen fixed left-0 top-0 flex flex-col gap-4 items-center justify-center backdrop-blur-sm">
              <div className="modal border-slate-400/20 rounded-lg border flex flex-col gap-4 items-center justify-center mx-auto p-2 2xl:p-5 w-full xs:w-[90%] sm:w-[75vw] md:w-[60vw] lg:w-[50vw] 2xl:w-[40vw] h-[60vh] relative">
                <button onClick={() => setImage(null)} className="absolute top-4 right-4 text-red-500 text-3xl font-bold">
                  &times;
                </button>
